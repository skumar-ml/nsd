...
class BriefsUpsellModal {
    ...
    constructor() {
        ...
        this.modalContainer = document.getElementById(
            "briefs-upsell-modal-container",
        )
        this.modalClose = document.getElementById("briefs-modal-close")
        this.briefEventsContainers = Array.from(
            document.querySelectorAll('[data-cart="briefEvents-container"]'),
        )
        this.briefEventsContainer = this.briefEventsContainers[0] || null
        this.setSelectedBriefEventsWrappers()
        this.briefCtaButtons = Array.from(
            document.querySelectorAll('[data-cart="add-brief-event"]'),
        )
        this.briefCtaButton = this.briefCtaButtons[0] || null
        this.briefCtaDefaultDisplay = this.briefCtaButton
            ? this.briefCtaButton.style.display
            : ""
        ...
        this.briefCheckboxIcons = {
            checked:
                "https://cdn.prod.website-files.com/6271a4bf060d543533060f47/691ecbd882b2c55efd083770_square-check%20(2).svg",
            unchecked:
                "https://cdn.prod.website-files.com/6271a4bf060d543533060f47/691ec94934f1278c4ad69157_square-check.svg",
        }
        this.briefEvents = []
    ...
    cacheBriefModalElements() {
        ...
        this.briefCtaButtons.forEach((button) => {
            if (!this.briefCtaDefaultDisplays.has(button)) {
                this.briefCtaDefaultDisplays.set(
                    button,
    ...
    setSelectedBriefEventsWrappers() {
        this.selectedBriefEventsWrappers = Array.from(
            document.querySelectorAll(".selected_brief_events"),
        )
        this.selectedBriefEventsWrapper =
            this.selectedBriefEventsWrappers[0] || null
    }
    // Returns array of selected brief events wrapper elements
    getSelectedBriefEventsWrappers() {
        if (
            Array.isArray(this.selectedBriefEventsWrappers) &&
            this.selectedBriefEventsWrappers.length
        ) {
            return this.selectedBriefEventsWrappers
        }
        return this.selectedBriefEventsWrapper
            ? [this.selectedBriefEventsWrapper]
            : []
    }
    ...
    formatCurrency(amount) {
        const numericAmount =
            parseFloat(
                typeof amount !== "undefined" && amount !== null ? amount : 0,
            ) || 0
    ...
    selectBriefEvent(eventId) {
        ...
        if (this.appliedBriefEvent) {
            const isSameBrief =
                String(this.appliedBriefEvent.eventId) ===
                String(selected.eventId)
    ...
    updateBriefAmount(nextSelection) {
        ...
        const currentBriefAmount =
            parseFloat(
                this.appliedBriefEvent && this.appliedBriefEvent.price
                    ? this.appliedBriefEvent.price
                    : 0,
            ) || 0
        const nextBriefAmount =
            parseFloat(
                nextSelection && nextSelection.price ? nextSelection.price : 0,
            ) || 0
        const updatedAmount =
    ...
    syncBriefCardsWithSelection() {
        ...
        const selectedId = this.selectedBriefEvent
            ? this.selectedBriefEvent.eventId
            : null
        containers.forEach((container) => {
            const cards = container.querySelectorAll("[data-brief-event-id]")
            cards.forEach((card) => {
                ...
                const isSelected =
                    selectedId && String(cardId) === String(selectedId)
                ...
                if (checkboxIcon) {
                    ...
                    checkboxIcon.setAttribute(
                        "src",
                        isSelected
                            ? this.briefCheckboxIcons.checked
                            : this.briefCheckboxIcons.unchecked,
                    )
                }
            })
        })
    ...
    buildSelectedBriefSummaryFragment() {
        ...
        const offeringType = creEl(
            "div",
            "dm-sans offering-type selected-brief-names",
        )
        ...
        const offeringRemove = creEl(
            "div",
            "dm-sans offering-remove brief-remove-btn",
        )
        ...
        offeringPrice.textContent = this.formatCurrency(
            this.selectedBriefEvent.price,
        )
    ...
    extractBriefFormat(title) {
        ...
        if (
            titleUpper.includes("LD + PF") ||
            titleUpper.includes("LD+PF") ||
            titleUpper.includes("BUNDLE")
        ) {
            return "LD + PF"
        }
    ...
    updateBriefCtaState() {
        ...
        this.briefCtaButtons.forEach((button) => {
            ...
            const isSameBrief =
                this.appliedBriefEvent &&
                this.selectedBriefEvent &&
                String(this.appliedBriefEvent.eventId) ===
                    String(this.selectedBriefEvent.eventId)
            ...
            if (isSameBrief) {
                ...
                const defaultStyles = this.briefCtaDefaultStyles
                    ? this.briefCtaDefaultStyles.get(button)
                    : null
                if (defaultStyles) {
                    button.style.pointerEvents =
                        defaultStyles.pointerEvents || ""
    ...
    clearBriefSelection() {
        ...
        if (totalAmountInput) {
            ...
            if (appliedSelection) {
                const updatedAmount =
                    currentAmount - (parseFloat(appliedSelection.price) || 0)
    ...
    findStoredBriefSelectionMatch() {
        ...
        if (
            storedId &&
            Array.isArray(this.briefEvents) &&
            this.briefEvents.length
        ) {
    ...
    getBriefEventsContainers() {
        if (
            Array.isArray(this.briefEventsContainers) &&
            this.briefEventsContainers.length
        ) {
...
class CheckOutWebflow extends BriefsUpsellModal {
    ...
    constructor(apiBaseUrl, memberData) {
        ...
        if (
            this.memberData.productType === "online_class" &&
            this.memberData.achAmount != null
        ) {
            var base = parseFloat(
                String(this.memberData.achAmount).replace(/,/g, ""),
            )
    ...
    hidePayLaterTab() {
        var payLaterTab = document.querySelector(
            "a.checkout-tab-link.pay-later, a.pay-later.new-checkout-tab-link, .pay-later.checkout-tab-link",
        )
        if (payLaterTab) {
    ...
    formatOnlineClassDisplayPrice(baseAmount, isCreditCard) {
        ...
        var amount = isCreditCard ? (baseAmount + 0.3) / 0.971 : baseAmount
        return (
            "$" +
            Number(amount)
                .toFixed(2)
    ...
    updateOnlineClassPriceForTab(tabEl) {
        ...
        if (this.$onlineClassBasePrice == null) {
            ...
            if (coreInput && coreInput.value) {
                var parsed = parseFloat(
                    String(coreInput.value).replace(/,/g, ""),
                )
        ...
        if (this.$onlineClassBasePrice == null) {
            ...
            if (fromDetails && fromDetails.textContent) {
                var parsed = parseFloat(
                    String(fromDetails.textContent).replace(/[$,]/g, ""),
                )
                if (!isNaN(parsed)) this.$onlineClassBasePrice = parsed
            }
        }
        if (this.$onlineClassBasePrice == null) {
            var ocPriceEl =
                document.querySelector(".oc-price") ||
                document.querySelector("[class*='oc-price']")
            if (ocPriceEl && ocPriceEl.textContent) {
                var parsed = parseFloat(
                    String(ocPriceEl.textContent).replace(/[$,]/g, ""),
                )
                if (!isNaN(parsed)) this.$onlineClassBasePrice = parsed
            }
        }
        if (this.$onlineClassBasePrice == null) {
            var selectedCard =
                document.querySelector(
                    ".checkout_offering-card-wrapper.selected",
                ) || document.querySelector("[data-class-offering-id].selected")
            if (selectedCard) {
                ...
                if (this.$onlineClassBasePrice == null) {
                    ...
                    if (priceEl && priceEl.textContent) {
                        var parsed = parseFloat(
                            String(priceEl.textContent).replace(/[$,]/g, ""),
                        )
                        if (!isNaN(parsed)) this.$onlineClassBasePrice = parsed
                    }
                }
        ...
        var isCreditCard = !!(
            tabEl &&
            (tabEl.classList.contains("credit-card-tab") ||
                (tabEl.querySelector &&
                    tabEl.querySelector(".credit-card-tab")))
        )
        var displayPrice = this.formatOnlineClassDisplayPrice(
            this.$onlineClassBasePrice,
            isCreditCard,
        )
        var numericAmount = isCreditCard
            ? (this.$onlineClassBasePrice + 0.3) / 0.971
            : this.$onlineClassBasePrice
        var formattedValue = Number(numericAmount)
            .toFixed(2)
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        var coreInput = document.getElementById("core_product_price")
        ...
        var orderSummaryWrappers = document.querySelectorAll(
            ".residential-order-summary-3",
        )
        for (var o = 0; o < orderSummaryWrappers.length; o++) {
            var onlineProgram = orderSummaryWrappers[o].querySelector(
                ".online-program.hide",
            )
    ...
    createCartList(suppData) {
        ...
        var checkboxS = creEl(
            "input",
            "w-checkbox-input core-checkbox suppCheckbox",
        )
        ...
        div.innerHTML =
            "Save $" +
            (suppData.disc_amount - suppData.amount).toFixed(2).toString()
        ...
        // price container
        var productPriceContainer = creEl(
            "div",
            "product-price-container hide-mobile",
        )
    ...
    displaySelectedSuppProgram(selectedIds) {
        var selectedSuppPro = document.getElementById(
            "selected_supplimentary_program",
        )
        var selectedSuppProMob = document.getElementById(
            "selected_supplimentary_program_mob",
        )
            ...
            checkboxEl.forEach((checkbox) => {
                ...
                if (programDetailId == suppId) {
                    ...
                    const addToCardButton =
                        parentDiv.querySelector(".add-to-card")
    ...
    updateOnlyTotalAmount() {
        ...
        if (totalPriceTextMob) {
            // added total price in dom element
            totalPriceTextMob.innerHTML = this.numberWithCommas(
                amount.toFixed(2),
            )
        }
    }
    ...
    updateAmount(checkEvent, amount) {
        ...
        if (checkEvent.checked) {
            ...
            var amountHtml =
                ...
                parseFloat(totalAmountInput.value) +
                parseFloat(amount)
            totalPriceText.innerHTML = this.numberWithCommas(
                amountHtml.toFixed(2),
            )
            if (totalPriceTextMob) {
                totalPriceTextMob.innerHTML = this.numberWithCommas(
                    amountHtml.toFixed(2),
    ...
    isOnlineClassPage() {
        return !!(
            document.querySelector(".checkout_offering-grid-container") ||
            document.querySelector("[data-class-offering-id]")
        )
    }
    ...
    initializeStripePayment(
        paymentType = "",
        checkoutID = "",
        $baseUrl = "createCheckoutUrlForOnlineClass",
    ) {
        return new Promise((resolve, reject) => {
            if (
                this.isOnlineClassPage() &&
                ($baseUrl === "createCheckoutUrlsByProgram" ||
                    $baseUrl === "updateStripeCheckoutDb")
            ) {
                reject(
                    new Error(
                        "Online class: use createCheckoutUrlForOnlineClass only",
                    ),
                )
                return
            }
            ...
            var data = {
                ...
                successUrl:
                    ...
                    "payment-confirmation?programId=" +
                    this.memberData.programId,
                cancelUrl: cancelUrl.href.includes("file:///")
                    ? "https://www.nsdebatecamp.com"
                    : cancelUrl.href,
                ...
                programStartDate: this.getProgramFormattedDate(
                    this.memberData.programStartDate,
                ),
                programEndDate: this.getProgramFormattedDate(
                    this.memberData.programEndDate,
                ),
                achAmount: parseFloat(
                    this.memberData.achAmount.replace(/,/g, ""),
                ),
                cardAmount: parseFloat(
                    this.memberData.cardAmount.replace(/,/g, ""),
                ),
                payLaterAmount: parseFloat(
                    this.memberData.payLaterAmount.replace(/,/g, ""),
                ),
    ...
    addSessionId() {
        ...
        if (localCheckOutData != undefined) {
            ...
            var checkout_student_details = document.getElementById(
    ...
    activateDiv(divId) {
        var divIdToStepId = {
            checkout_program: "program",
            checkout_student_details: "student-details",
            checkout_payment: "pay-deposite",
        }
        var divIds = [
            "checkout_program",
            "checkout_student_details",
        ...
        var allStepLis = document.querySelectorAll(
            ".stepper-container .stepper li, .stepper-container ul li, ul.stepper li, ul li.step",
        )
        ...
        if (!activeStepEl && stepId === "pay-deposite")
            activeStepEl = document.getElementById("select-class-and-pay")
        if (activeStepEl) {
            ...
            if (idx < 0)
                stepOrder = ["program", "student-details", "pay-deposite"]
            idx = stepOrder.indexOf(stepId)
            if (idx >= 0 && allStepLis[idx])
                allStepLis[idx].classList.add("active")
        }
        if (
            divId === "checkout_payment" &&
            typeof this.updateOnlineClassPriceForTab === "function" &&
            this.isOnlineClassPage()
        ) {
            var activeTab =
                document.querySelector(".checkout-tab-link.w--current") ||
                document.querySelector(".checkout-tab-link")
            if (activeTab) {
        ...
        if (
            divId === "checkout_payment" &&
            typeof this.updateCheckoutPaymentGreeting === "function"
        ) {
    ...
    addEventForPrevNaxt() {
                ...
                if (form.valid()) {
                    ...
                    if (initialCheckout) {
                        initialCheckout.then(() => {
                            var checkoutData = [
                                $this.$checkoutData.achUrl,
                                $this.$checkoutData.cardUrl,
                                $this.$checkoutData.payLaterUrl,
                            ]
                            $this
                                .updateStudentDetails(checkoutData)
                                .then(() => {
                    ...
                    var sliderData = (this.$suppPro = $this.$suppPro.filter(
                        (i) => i.programDetailId != 21,
                    ))
                    if (sliderData.length > 0) {
                        $this.hideShowDivById(
                            "checkout-supplimentary-data-2",
                            "block",
                        )
                        $this.hideShowDivById(
                            "checkout-supplimentary-data-desktop",
                            "block",
                        )
                    }
                    ...
                    const canShowBriefUpsell = Boolean(
                        $this.memberData &&
                        $this.memberData.isAdmin &&
                        !$this.$isAboundedProgram,
                    )
        ...
        let editStudentEl = document.querySelectorAll(
            "[data-student-info='edit']",
        )
    ...
    reinitializePaymentTab() {
        var bankTransferTab =
            document.getElementsByClassName("bank-transfer-tab")[0]
        if (bankTransferTab) bankTransferTab.click()
        var payNowLink = document.getElementById("pay-now-link")
        if (payNowLink && payNowLink.closest("div"))
            payNowLink.closest("div").style.display = "none"
        var payNowLink2 = document.getElementById("pay-now-link-2")
        if (payNowLink2 && payNowLink2.closest("div"))
            payNowLink2.closest("div").style.display = "none"
        var payNowLink3 = document.getElementById("pay-now-link-3")
        if (payNowLink3 && payNowLink3.closest("div"))
            payNowLink3.closest("div").style.display = "none"
    ...
    handlePaymentEvent() {
        ...
        if (ach_payment) {
            ach_payment.addEventListener("click", function () {
                ...
                if ($this.isOnlineClassPage()) {
                    ...
                    ach_payment.disabled = true
                    $this
                        .createCheckoutUrlForOnlineClass(
                            $this.$selectedClassOfferingId,
                        )
                        .then(function (urls) {
                            var goUrl =
                                urls && urls.achUrl
                ...
                var myInterval1 = setInterval(function () {
                    ...
                    var initialCheckout = $this.initializeStripePayment(
                        "us_bank_account",
                        $this.$checkoutData && $this.$checkoutData.checkoutId,
                        "updateStripeCheckoutDb",
                    )
                    if (initialCheckout) {
        ...
        if (paylater_payment) {
            paylater_payment.addEventListener("click", function () {
                ...
                if (
                    $this.isOnlineClassPage() &&
                    (!$this.$checkoutData || !$this.$checkoutData.payLaterUrl)
                ) {
                    alert("Please select a class offering first.")
                    return
                }
    ...
    updateDefaultCheckbox() {
        ...
        for (var i = 0; i < cPrice.length; i++) {
            let price = parseFloat(
                cPrice[i].innerHTML.replace(/,/g, "").replace(/\$/g, ""),
            )
    ...
    checkBackButtonEvent() {
        ...
        if (
            (returnType == "back" ||
                ibackbutton.value == 1 ||
                this.isWithinAWeek(paymentData.createdOn)) &&
    ...
    setUpBackButtonTab() {
        ...
        if ($this.checkBackButtonEvent() && checkoutJson != undefined) {
            ...
            if (paymentData.supplementaryProgramIds.length > 0) {
                var SuppCheckbox =
                    document.getElementsByClassName("suppCheckbox")
                var uniqueProgramCategoryIds = new Set()
                for (let i = 0; i < SuppCheckbox.length; i++) {
                    var checkBoxProgramdetailid =
                        SuppCheckbox[i].getAttribute("programdetailid")

                    if (
                        !uniqueProgramCategoryIds.has(checkBoxProgramdetailid)
                    ) {
                        uniqueProgramCategoryIds.add(checkBoxProgramdetailid)
                        if (
                            paymentData.supplementaryProgramIds.includes(
                                checkBoxProgramdetailid,
                            )
                        ) {
                            SuppCheckbox[i].checked = true
                            $this.updateAmount(
                                SuppCheckbox[i],
                                SuppCheckbox[i].value,
                            )

                            var elementSelector =
                                ".supp_program_" + checkBoxProgramdetailid
                            var matchedAddCartBtn =
                ...
                if (paymentData.supplementaryProgramIds.length > 0) {
                    this.displaySelectedSuppProgram(
                        paymentData.supplementaryProgramIds,
                    )
                }
    ...
    async renderPortalData(memberId) {
        try {
            ...
            var activeStep =
                document.querySelector(".stepper-container ul li.active") ||
    ...
    setupClassOfferingCardClicks() {
        ...
        function handleClassOfferingClick(e) {
            var wrapper =
                e.target.closest(".checkout_offering-card-wrapper") ||
                e.target.closest("[data-class-offering-id]")
            if (!wrapper) return
            ...
            if (container) {
                container
                    .querySelectorAll(
                        ".checkout_offering-card-wrapper, [data-class-offering-id]",
                    )
                    .forEach(function (el) {
    ...
    createCheckoutUrlForOnlineClass(classOfferingId) {
        var apiUrl = `${PAYMENT_API_BASE}/createCheckoutUrlForOnlineClass`
        ...
        var email =
            this.memberData && this.memberData.email
                ? this.memberData.email.toLowerCase().trim()
                : ""
        var memberId =
            this.memberData && this.memberData.memberId
                ? String(this.memberData.memberId).trim()
                : ""
        var studentEmail = studentEmailEl
            ? studentEmailEl.value.trim().toLowerCase()
            : ""
        var cancelUrl = new URL(window.location.href)
        ...
        var body = {
            ...
            student: {
                email: studentEmail,
                first_name: studentFirstNameEl
                    ? studentFirstNameEl.value.trim()
                    : "",
                last_name: studentLastNameEl
                    ? studentLastNameEl.value.trim()
                    : "",
        ...
        var $this = this
        return NSDAuth.authFetch(apiUrl, {
            method: "POST",
            ...
            .then(function (data) {
                ...
                if (achUrl || cardUrl || payLaterUrl) {
                    ...
                    $this.$initCheckout = true
                    $this.updateCheckoutUrlHiddenInputs(
                        achUrl,
                        cardUrl,
                        payLaterUrl,
                    )
    ...
    updateCheckoutUrlHiddenInputs(achUrl, cardUrl, payLaterUrl) {
        ...
        var container =
            document.getElementById("checkout_student_details") ||
            document.getElementById("checkout_payment") ||
            document.body
    ...
    eventForPayNowBtn() {
        ...
        function handlePayNowClick(e, payNowEl) {
            ...
            if ($this.isOnlineClassPage() && $this.$selectedClassOfferingId) {
                ...
                var isCard = !!(
                    activeTab && activeTab.classList.contains("credit-card-tab")
                )
                $this
                    .createCheckoutUrlForOnlineClass(
                        $this.$selectedClassOfferingId,
                    ...
                    .catch(function (err) {
                        ...
                        payNowEl.innerHTML =
                            payNowEl.getAttribute("data-default-text") ||
                            "Checkout"
                        alert(
                            err && err.message
                                ? err.message
        ...
        for (var i = 0; i < allTabs.length; i++) {
            ...
            tab.addEventListener("click", function () {
                ...
                if (payNowLinkMo && payNowLinkMo.closest("div"))
                ...
                const hasBriefSelection = Boolean(
                    $this.appliedBriefEvent || $this.selectedBriefEvent,
                )
                ...
                if (
                    (hasBriefSelection || hasSuppSelections) &&
                    typeof $this.updateOnlyTotalAmount === "function"
                ) {
    ...
    async updateOldStudentList() {
        ...
        if (
            !this.memberData ||
            this.memberData.memberId == null ||
            this.memberData.memberId === ""
        ) {
            selectBox.innerHTML =
                '<option value="">Please sign in or refresh</option>'
            return
        }
        try {
            const rawData = await this.fetchData(
                AUTH_API_BASE,
                "/getAllPreviousStudents/" + this.memberData.memberId + "/true",
            )
            // API may return array or { data: [] } / { students: [] }
            const data = Array.isArray(rawData)
                ? rawData
                : (rawData && (rawData.data || rawData.students)) || []
            ...
            const filterData = data
                .filter(
                    (item, index, self) =>
                        ...
                        index ===
                            self.findIndex(
                                (obj) => obj.studentEmail === item.studentEmail,
                            ),
                )
                .sort(function (a, b) {
    ...
    showUpSellModal(modal) {
        ...
        document
            .querySelector(".upsell-modal-bg")
            .setAttribute("aria-hidden", "false")
    }
    checkUpSellModalOpen() {
        let isOpen = false
        const addToCartButtons = document.querySelectorAll(
            ".add-to-card.upsell_add_to_card",
        )
            ...
            button.addEventListener("click", function (event) {
                ...
                if (parent) {
                    ...
                    if (checkbox && !checkbox.checked) {
                        ...
                        var programDetailId =
                            checkbox.getAttribute("programdetailid")
                        ...
                        setTimeout(() => {
                            const modal =
                                document.getElementById("upsell-modal-1")
                            $this.hideUpSellModal(modal)
                        }, 100)
                    }
                }
                ...
                if (this.classList.contains("care_package_add_to_card")) {
                    const _care_package_add_to_card = document.querySelectorAll(
                        ".care_package_add_to_card",
                    )
    ...
    async displaySupplementaryProgram() {
        ...
        variant_type =
            variant_type != undefined || variant_type != null
                ? variant_type
                : ""
        let prep_week_data = apiData.filter((i) =>
            i.label.toLowerCase().includes(prep_week_searchText.toLowerCase()),
        )
        let tutoring_data = apiData.filter((i) =>
            i.label
                .toLowerCase()
                .includes(tutoring_week_searchText.toLowerCase()),
        )
            ...
            if (variant_type == 1) {
                apiData = apiData.filter(
                    (i) =>
                        !i.label
                            .toLowerCase()
                            .includes(prep_week_searchText.toLowerCase()),
                )
            } else {
                apiData = apiData.filter(
                    (i) =>
        ...
        apiData.forEach((item) => {
            ...
            // Modal Content Update
            const modalSingleContent = this.displayModalSuppProgram(
                item,
                "modal",
            )
    ...
    updateUpSellModal(prep_week_data, tutoring_data, care_package_data) {
        if (prep_week_data.length > 0) {
            ...
            if (tpwAmount != undefined) {
                tpwAmount.innerHTML =
                    "$" + parseFloat(prep_week_data[0].amount).toFixed(2)
            }
            var tpwSaveAmount = document.getElementById("tpw-save-amount")
            if (tpwSaveAmount != undefined) {
                tpwSaveAmount.innerHTML =
                    "$" +
                    parseFloat(
                        prep_week_data[0].disc_amount -
                            prep_week_data[0].amount,
                    ).toFixed(2)
            }
            var tpwDescAmount = document.getElementById("tpw-desc-amount")
            if (tpwDescAmount != undefined) {
                tpwDescAmount.innerHTML =
                    "$" + parseFloat(prep_week_data[0].disc_amount).toFixed(2)
            }
            var upsellTpwProgranId =
                document.getElementById("upsellTpwProgranId")
            if (upsellTpwProgranId != undefined) {
                upsellTpwProgranId.setAttribute(
                    "programdetailid",
                    prep_week_data[0].programDetailId,
                )
                upsellTpwProgranId.value = prep_week_data[0].amount
            }
            // For mobile upsellTpwProgranId-1
            var upsellTpwProgranIdMob = document.getElementById(
                "upsellTpwProgranId-1",
            )
            if (upsellTpwProgranIdMob != undefined) {
                upsellTpwProgranIdMob.setAttribute(
                    "programdetailid",
                    prep_week_data[0].programDetailId,
                )
            ...
            let tpwReadMore = document.querySelectorAll(
                "[upsell-modal='tpw_read-more']",
            )
            if (tpwReadMore.length > 0) {
                tpwReadMore.forEach((read_more_link) => {
                    read_more_link.href =
                        this.memberData.site_url + "topic-prep-week"
                })
            }
        }
        ...
        if (tutoring_data.length > 0) {
            ...
            if (tutoAmount != undefined) {
                tutoAmount.innerHTML =
                    "$" + parseFloat(tutoring_data[0].amount).toFixed(2)
            }
            var tutoSaveAmount = document.getElementById("tuto-save-amount")
            if (tutoSaveAmount != undefined) {
                tutoSaveAmount.innerHTML =
                    "$" +
                    parseFloat(
                        tutoring_data[0].disc_amount - tutoring_data[0].amount,
                    ).toFixed(2)
            }
            var tutoDescAmount = document.getElementById("tuto-desc-amount")
            if (tutoDescAmount != undefined) {
                tutoDescAmount.innerHTML =
                    "$" + parseFloat(tutoring_data[0].disc_amount).toFixed(2)
            }
            var upsellTutoProgranId = document.getElementById(
                "upsellTutoProgranId",
            )
            if (upsellTutoProgranId != undefined) {
                upsellTutoProgranId.setAttribute(
                    "programdetailid",
                    tutoring_data[0].programDetailId,
                )
                upsellTutoProgranId.value = tutoring_data[0].amount
            }
            // Mobile div id upsellTutoProgranId-1
            var upsellTutoProgranIdMob = document.getElementById(
                "upsellTutoProgranId-1",
            )
            if (upsellTutoProgranIdMob != undefined) {
                upsellTutoProgranIdMob.setAttribute(
                    "programdetailid",
                    tutoring_data[0].programDetailId,
                )
            ...
            //tutoring title
            let tutoringTitle = document.querySelector(
                "[upsell-modal='tutoring-title']",
            )
            if (tutoringTitle != undefined) {
                tutoringTitle.innerHTML = tutoring_data[0].label
            }

            let tutoringReadMore = document.querySelectorAll(
                "[upsell-modal='tutoring_read-more']",
            )
            if (tutoringReadMore.length > 0) {
                tutoringReadMore.forEach((read_more_link) => {
                    read_more_link.href =
                        this.memberData.site_url + "debate-tutoring"
                })
            }
        }
        // Care Package Data Update
        if (care_package_data != undefined) {
            let carePackagePrice = document.querySelectorAll(
                "[data-care-package='price']",
            )
            ...
            let carePackageCheckbox = document.querySelectorAll(
                "[data-care-package='checkbox']",
            )
            if (carePackageCheckbox.length > 0) {
                carePackageCheckbox.forEach((cp_checkbox) => {
                    cp_checkbox.setAttribute(
                        "programdetailid",
                        care_package_data.programDetailId,
                    )
    ...
    displaySingleSuppProgram(item, size, slideDiv) {
        ...
        input.addEventListener("change", function () {
            this.checked
                ? slideDiv.classList.add("border-red")
                : slideDiv.classList.remove("border-red")
        ...
        campNameDiv.setAttribute(
            "for",
            size + item.label.replace(/\s+/g, "-").toLowerCase(),
        )
        ...
        saveDiv2.textContent =
            "$" +
            (parseFloat(item.disc_amount) - parseFloat(item.amount)).toFixed(2)
    ...
    getVariant() {
        ...
        let tutoringUpSellModal = document.querySelector(
            ".upsell-modal-container.tutoring",
        )
        if (
            tutoringUpSellModal &&
            window.getComputedStyle(tutoringUpSellModal).display != "none"
        ) {
            variant = 2
        }
        return variant
    ...
    activeBreadCrumb(activeId) {
        ...
        if (!activeEl && activeId === "pay-deposite")
            activeEl = document.getElementById("select-class-and-pay")
        var breadCrumbList = document.querySelectorAll(
            ".stepper-container .stepper li, .stepper-container ul li, ul.stepper li, ul li.step",
        )
        ...
        if (activeEl) {
            ...
            if (idx >= 0 && breadCrumbList[idx])
                breadCrumbList[idx].classList.add("active")
        }
        var stepToDivId = {
            program: "checkout_program",
            "student-details": "checkout_student_details",
    ...
    newDisplaySingleSuppProgram(item, size, slideDiv) {
        ...
        originalPrice.textContent =
            "$" + parseFloat(item.disc_amount).toFixed(2)
    ...
    displayModalSuppProgram(item, type = "banner", size = "desktop") {
        ...
        var buyNowBtn = creEl(
            "a",
            "main-button red add-to-card supp-program w-button " + programClass,
        )
        buyNowBtn.href = "#"
        ...
        if (benefits.length > 0) {
            ...
            learnMoreBtn.addEventListener("click", function (event) {
                event.preventDefault()
                $this.$selectedProgram = item
                const suppProgramsModal =
                    document.getElementById("suppProgramsModal")
                if (type == "banner") {
                    $this.hideShowModalContent(item)
        ...
        percentOff.innerHTML =
            "$" +
            discount +
            '<span class="off-text-shadow-right-white"> OFF</span>'
    ...
    displayStudentInfo(display) {
        document
            .querySelectorAll(".student-info-container")
        ...
        if (localCheckOutData != undefined) {
            ...
            let firstNameEls = document.querySelectorAll(
            ...
            let lastNameEls = document.querySelectorAll(
