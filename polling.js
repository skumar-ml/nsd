...
class Polling {
    ...
    displayUnreadPolling() {
        ...
        var unreadPolling = this.messageData.filter(
        ...
        if (searchFilter.value) {
            ...
            messageData = messageData.filter(function (el) {
                return (
                    ...
                    condition.test(el.activityType) ||
    ...
    viewDownLoadedFile(fileLink, item) {
        ...
        var a = creEl(
            "a",
            "downloadLink iframe-lightbox-link poll-btn main-button red w-button",
        )
    ...
    showListPage() {
        ...
        var pollingHeading =
            document.getElementsByClassName("polling-heading")[0]
    ...
    readApiCall(messageId) {
        ...
        xhr.open(
            "POST",
            "https://3yf0irxn2c.execute-api.us-west-1.amazonaws.com/dev/camp/isReadPolledNotification",
            true,
        )
    ...
    makePollingFilter() {
        var pollingFilter = document.getElementById("polling-filter")
        /*Filter*/
        var pollingHeader = creEl(
            "div",
            "polling-header w-layout-grid grid-3",
            "polling-header",
        )
    ...
    getPollingEditData() {
        ...
        xhr.open(
            "GET",
            "https://3yf0irxn2c.execute-api.us-west-1.amazonaws.com/dev/camp/getPollingDetails/" +
                $this.webflowMemberId,
            true,
        )
        xhr.withCredentials = false
        ...
        xhr.onload = function () {
            ...
            if (responseText.length > 0) {
                var currentFindData = responseText.find(
                    (item) => item.oid == currentDetailsData.oid,
                )
