...
class BriefsEventsCheckout {
    ...
    async getBriefsAndEvents() {
        ...
        try {
            ...
            if (response) {
                ...
                if (
                    this.data.briefs.length === 0 &&
                    this.data.briefEvents.length === 0
                ) {
                    this.showError(
                        "No briefs or events are currently available.",
                    )
                } else {
                    ...
                    if (
                        this.selectedAccordion === "annual-events-accordion" &&
                        this.data.briefEvents.length > 0
                    ) {
                        ...
                        this.selectedAccordion === "single-briefs-accordion" &&
                        this.data.briefs.length > 0
                    ) {
                        ...
                        if (this.data.briefs.length > 0) {
                            ...
                            this.renderBriefs(this.data.briefs)
                            setTimeout(() => {
                                this.fixAccordionHeight(
                                    "single-briefs-accordion",
                                )
                            }, 100)
    ...
    renderBriefs(briefs) {
        const container = document.querySelector(
            '[data-brief="single-briefs-accordion"] .briefs-accordion-item-body',
        )
        if (!container) {
    ...
    renderEvents(events) {
        const container = document.querySelector(
            '[data-brief="annual-events-accordion"] .annual-subs-item-body',
        )
        if (!container) {
    ...
    showLoading() {
        ...
        const annualContainer = document.querySelector(
            '[data-brief="annual-events-accordion"] .annual-subs-item-body',
        )
...
`
        ...
        card.addEventListener("click", () =>
            this.toggleBriefSelection(brief.briefId, card),
        )
    ...
    createEventCard(event, isSelected = false) {
        ...
        const featuresHTML = event.features
            ? event.features
                  .map(
                      (feature) => `
...
`
        ...
        // Add click handler
        card.addEventListener("click", () =>
            this.toggleEventSelection(event.eventId, card),
        )
        ...
        const briefsTotal = this.selectedBriefs.reduce((sum, briefId) => {
            ...
            if (card) {
                ...
                if (priceElement) {
                    ...
                    const finalPrice = isCreditCardSelected
                        ? this.calculateCreditCardAmount(basePrice)
                        : basePrice
                    console.log(
                        `Brief ${briefId}: basePrice=${basePrice}, finalPrice=${finalPrice}`,
                    )
            ...
            if (card) {
                ...
                if (priceElement) {
                    ...
                    const finalPrice = isCreditCardSelected
                        ...
                        : basePrice
                    console.log(
                        `Event ${eventId}: basePrice=${basePrice}, finalPrice=${finalPrice}`,
                    )
                    return sum + finalPrice
                }
            }
    ...
    updateTotal() {
        ...
        if (isCreditCardSelected) {
            ...
            console.log(
                "Individual amounts with credit card fees:",
                individualAmounts,
            )
        }
    }
...
`
    ...
    setupAccordion() {
        ...
        const annualAccordion = document.querySelector(
            '[data-brief="annual-events-accordion"]',
        )
        const singleAccordion = document.querySelector(
            '[data-brief="single-briefs-accordion"]',
        )

        if (annualAccordion) {
            const annualHeader = annualAccordion.querySelector(
                ".annual-subs-item-header",
            )
            if (annualHeader) {
        ...
        if (singleAccordion) {
            const singleHeader = singleAccordion.querySelector(
                ".briefs-accordion-item-header",
            )
            if (singleHeader) {
    ...
    selectAccordion(accordionType, type = "") {
        ...
        if (annualAccordion) {
            const annualBody = annualAccordion.querySelector(
                ".annual-subs-item-body",
            )
            const annualRadio = annualAccordion.querySelector(
                'input[type="radio"]',
            )
            const annualIcon = annualAccordion.querySelector(
            ...
            if (accordionType === "annual-events-accordion") {
                ...
                annualAccordion
                    .querySelector(".annual-subs-accordion-item")
                    .classList.add("open")
        ...
        if (singleAccordion) {
            const singleBody = singleAccordion.querySelector(
                ".briefs-accordion-item-body",
            )
            const singleRadio = singleAccordion.querySelector(
                'input[type="radio"]',
            )
            const singleIcon = singleAccordion.querySelector(
                ".accordion-toggle-icon",
            )
            ...
            if (accordionType === "single-briefs-accordion") {
                ...
                singleAccordion
                    .querySelector(".briefs-accordion-item")
                    .classList.add("open")
    ...
    toggleCurrentAccordion(accordionType) {
        ...
        if (accordionType === "annual-events-accordion") {
            ...
            if (annualAccordion) {
                ...
                const accordionItem = annualAccordion.querySelector(
                    ".annual-subs-accordion-item",
                )
                ...
                if (isOpen) {
                    ...
                    if (annualBody) {
                        ...
                        setTimeout(() => {
                            annualBody.style.maxHeight =
                                annualBody.scrollHeight + "px"
                        }, 10)
                    }
                    accordionItem.classList.add("open")
                    if (annualRadio) annualRadio.checked = true
                    if (annualIcon)
                        annualIcon.style.transform = "rotate(180deg)"
                }
            }
        } else if (accordionType === "single-briefs-accordion") {
            const singleAccordion = document.querySelector(
            ...
            if (singleAccordion) {
                ...
                const accordionItem = singleAccordion.querySelector(
                    ".briefs-accordion-item",
                )
                ...
                if (isOpen) {
                    ...
                    if (singleBody) {
                        ...
                        // Use setTimeout to ensure content is fully rendered before calculating height
                        setTimeout(() => {
                            singleBody.style.maxHeight =
                                singleBody.scrollHeight + "px"
                        }, 10)
                    }
                    accordionItem.classList.add("open")
                    if (singleRadio) singleRadio.checked = true
                    if (singleIcon)
                        singleIcon.style.transform = "rotate(180deg)"
                }
            }
        }
    }
    ...
    setInitialState() {
        ...
        const paymentSection = document.querySelector(
            '[data-briefs-checkout="select-payment-method"]',
        )
        if (paymentSection) {
        ...
        const briefsSection = document.querySelector(
            '[data-briefs-checkout="select-briefs"]',
        )
        if (briefsSection) {
    ...
    setupPaymentMethodTabs() {
        const paymentTabs = document.querySelectorAll(
            ".payment-cards-tabs-menu .w-tab-link",
        )
    ...
    setupPayNowButtons() {
        ...
        const payNowElements = document.querySelectorAll(
            '[data-briefs-checkout="pay-now"]',
        )
    ...
    showPaymentMethod() {
        ...
        if (
            this.selectedBriefs.length === 0 &&
            this.selectedEvents.length === 0
        ) {
            alert(
                "Please select at least one brief or event before proceeding to payment.",
            )
            return
        }
    ...
    updateOrderDetails() {
        const emptyOrderDetails = document.querySelector(
            '[data-briefs-checkout="empty-order-details"]',
        )
        const briefsOrderDetails = document.querySelector(
            '[data-briefs-checkout="briefs-order-details"]',
        )
    ...
    updateItemsListInOrderDetails() {
        const briefsContainer = document.querySelector(
        ...
        const existingItems = briefsContainer.parentElement.querySelectorAll(
            ".brief-flex-wrapper",
        )
        ...
        this.selectedBriefs.forEach((briefId) => {
            ...
            if (card) {
                const title = card
                    .querySelector(".pdf-briefs-title")
                    .textContent.trim()
                const basePriceText = card
                    .querySelector(".pdf-brief-price")
                    .textContent.trim()
                const basePrice = parseFloat(
                    basePriceText.replace("$", "").replace("\n", "").trim(),
                )
...
`
                ...
                briefsContainer.parentElement.insertBefore(
                    itemElement,
                    briefsContainer.nextSibling,
                )
    ...
    updateOrderTotalDisplay(total) {
        const totalElement = document.querySelector(
            '[data-briefs-checkout="briefs-order-details"] .total-price-bold',
        )
        if (totalElement) {
    ...
    updateOrderTotal() {
        ...
        console.warn(
            "updateOrderTotal() is deprecated. Use updateOrderTotalDisplay() instead.",
        )
    }
    ...
    isCreditCardSelected() {
        ...
        const isSelected =
            creditCardTab && creditCardTab.classList.contains("w--current")
    ...
    validatePaymentData() {
        ...
        if (!this.data.accountEmail) {
            alert(
                "Account email is required for payment. Please check your account settings.",
            )
            return false
        }
        ...
        if (paymentSection && paymentSection.style.display !== "none") {
            ...
            const isCreditCardSelected = this.isCreditCardSelected()
            const isBankTransferSelected = document.querySelector(
                ".bank-transfer-tab.w--current",
            )
    ...
    payNow() {
        ...
        const payNowButtons = document.querySelectorAll(
            '[data-briefs-checkout="pay-now"], #pay-now-link-3',
        )
