...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var product = urlPar.get('productType') || urlPar.get('producttype') || 'online-class'
  ...
  var class_detail_id = class_detail_id_raw ? toMongoObjectId(class_detail_id_raw) : ''
  var session_detail_id = session_detail_id_raw ? toMongoObjectId(session_detail_id_raw) : ''
  ...
  ;(function () {
    var signUpWrapper = document.querySelector(".sign-up-form-wrapper-mb-20.w-form") || document.querySelector(".sign-up-form-wrapper-mb-20")
    ...
    function syncActive() {
      ...
      if (!isBlock) return
      if (!createAccountNew.classList.contains("active") ||
    ...
    wrapperObserver.observe(signUpWrapper, { attributes: true, attributeFilter: ["style", "class"] })

    if (studentDetails) {
      var stepObserver1 = new MutationObserver(syncActive)
      stepObserver1.observe(studentDetails, { attributes: true, attributeFilter: ["class"] })
    }
    if (payDeposite) {
      var stepObserver2 = new MutationObserver(syncActive)
      stepObserver2.observe(payDeposite, { attributes: true, attributeFilter: ["class"] })
    }
  })()
  ...
  async function getClassOfferings() {
    ...
    document.querySelectorAll(".online-program p.dm-sans.font-14.bold")
      .forEach(el => el.textContent = programText)

    document.querySelectorAll(".oc-price")
      .forEach(el => el.textContent = priceText)

    const gridContainer = document.querySelector(".checkout_offering-grid-container")
    ...
    schedules.forEach(function (schedule) {
      ...
      const availableSpots = schedule.available_spots != null
      ...
      if (stateClass !== "full") {
        card.addEventListener("click", function () {
          document.querySelectorAll(".checkout_offering-card")
            .forEach(el => el.classList.remove("slot-selected"))
          document.querySelectorAll(".checkout_offering-card-wrapper")
          ...
          if (typeof renderer !== "undefined" && renderer && renderer.memberData) {
            renderer.memberData.programCategoryId = class_offering_id
          }
          const paymentContainer = document.querySelector(".checkout-page_payment-option-container")
  ...
  if (core_product_price && accountEmail && jQuery && jQuery.fn && jQuery.fn.validate) {
    ...
    jQuery.validator.addMethod("notEqual", function (value, element, param) {
      return this.optional(element) ||
        (value.replace(/\s/g, '').toLowerCase() !== param.replace(/\s/g, '').toLowerCase() &&
         value.replace(/\./g, '').toLowerCase() !== param.replace(/\./g, '').toLowerCase())
