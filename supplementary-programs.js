...
Brief Logic: Fetches supplementary programs data from API and renders them in a list with filtering and pagination controls. Handles program selection and displays program details.
...
class SupplementaryForm {
    ...
    view() {
        ...
        if (this.$formslist) {
            ...
            var supplementaryDiv = document.getElementById(
                "supplementary-" + $this.currentIndex,
            )
            supplementaryDiv.innerHTML = ""
            var $tabNo = 1
            $this.$totalForm = 0
            parentForm.sort(function (r, a) {
                return r.sequence - a.sequence
            }) // order form categories via order specified in MongoDB
            // for every form category in parentForm, start building supplementary
            parentForm.forEach((form) => {
                var supplementaryContainerDiv = creEl(
                    "div",
                    "supplementary-container",
                    "supplementary-container-" + $tabNo + $this.currentIndex,
                )
                var labelDiv = creEl(
                    "div",
                    "label slabel-" + $this.currentIndex,
                )
                ...
                form.forms.forEach((cForm) => {
                    ...
                    if (is_live) {
                        if (editable) {
                            ...
                            if (this.$isLiveProgram) {
                                ...
                                formLink.href =
                                    "https://www.jotform.com/submission/" +
                                    dbData.submissionId
                            }
                        } else {
                            formLink.href = cForm.formId
                    ...
                    formLink.className =
                        is_live && window.innerWidth > 1200
                ...
                supplementaryContainerDiv.prepend(
                    labelDiv,
                    supplementaryContentDiv,
                )
    ...
    viewService() {
        ...
        service.innerHTML =
            this.$programDetail.programName +
            " " +
            this.$programCategory.programCategoryName
    }
    /**
    ...
    checkAllForms(forms) {
        if (forms) {
            ...
            var completedFormsId = this.$completedForm.map((formItem) =>
                formItem.formId.toString(),
            )
            const compareForm = completedFormsId.filter(
                (obj) => formsId.indexOf(obj) !== -1,
            )
            const uniqueform = compareForm.filter(
                (value, index, self) => self.indexOf(value) === index,
            )
            return (
                formsId.length === uniqueform.length &&
                formsId.every((val) => uniqueform.includes(val))
            )
        }
    }
    /**
    ...
    renderSupplementaryHeader() {
        ...
        let percentageAmount = this.$completedForm.length
            ...
            : 0
        let supplementaryFooter = document.getElementById(
            "supplementary-footer-" + this.currentIndex,
        )
        ...
        protext.innerHTML =
            ...
            this.currentIndex +
            "'>0%</span> / " +
            this.$completedForm.length +
            " of " +
            this.$totalForm +
            " forms complete &nbsp;"
        let progressContainer = document.createElement("div")
        progressContainer.className =
            "progress-container progress-container-" + this.currentIndex
        progressContainer.setAttribute(
            "data-percentage",
            Math.round(percentageAmount) ? Math.round(percentageAmount) : 0,
        )
        ...
        if (deadlineDate) {
            footerText.innerHTML =
                "Please complete these required forms prior to: " +
    ...
    setPercentage() {
        const progressContainer = document.querySelector(
            ".progress-container-" + this.currentIndex,
        )
        const percentage =
            progressContainer.getAttribute("data-percentage") + "%"
        const progressEl = progressContainer.querySelector(
            ".progress-" + this.currentIndex,
        )
        progressEl.style.width = percentage
        const percentageValue = document.querySelector(
            ".percentage-value-" + this.currentIndex,
        )
    ...
    programDates() {
        ...
        var program_dates = document.getElementById(
            "program_dates-" + this.currentIndex,
        )
        program_dates.innerHTML =
            "Supplementary Program Dates: " +
    ...
    initiateSupplementary() {
        const supplementary = document.getElementsByClassName(
            "slabel-" + this.currentIndex,
        )
...
class SupplementaryProgram {
    ...
    constructor(webflowMemberId, accountEmail) {
        this.webflowMemberId = webflowMemberId
        this.accountEmail = accountEmail
        this.callApi() // renders data for each stab
    }
    ...
    makeList(apiData, selectProgram) {
        ...
        var studentList = this.creEl(
            "select",
            "student-select-list w-select",
            "student-select-list",
        )
        //Add default option
        ...
        apiData.forEach((item) => {
            ...
            option.text =
                item.studentDetail.studentName.first +
                " " +
                item.studentDetail.studentName.last +
                " (" +
                item.programDetail.programName +
                "-" +
                item.programCategory.programCategoryName +
                ")"
    ...
    saveLocalStorageData(studentSelectListValue) {
        if (Array.isArray(this.$apiData)) {
            var currentStudentData = this.$apiData.filter(
                (item) =>
                    item.studentDetail.uniqueIdentification ==
                    studentSelectListValue,
            )
            localStorage.setItem(
                "supStuEmail",
                JSON.stringify(currentStudentData[0].studentDetail),
            )
        }
    }
    ...
    selectProgram($this) {
        ...
        var supplementaryProgramList = document.getElementById(
            "supplementary-program-list",
        )
        if (!studentSelectList.value) {
            ...
            Array.from(supprolink).forEach((link) => {
                ...
                link.setAttribute(
                    "href",
                    linkUrl[0] + "?productType=supplementary",
                )
            })
            let programName =
                studentSelectList.options[
                    studentSelectList.selectedIndex
                ].getAttribute("data-programName")
...
class SupplementaryTabs {
    ...
    viewstabs(responseText) {
        ...
        responseText.forEach((formData, index) => {
            ...
            var activeliClass =
                currentIndex == 1 && is_single && formData.formList.length > 0
                    ? "stab_li"
                    : "stab_li"
            ...
            stabsE.innerHTML =
                formData.studentDetail.studentName.first +
                " " +
                formData.studentDetail.studentName.last +
                " - " +
                formData.programDetail.programName
            ...
            if (formData.formList.length > 0) {
                ...
                var supplementary = creEl(
                    "div",
                    "supplementary",
                    "supplementary-" + currentIndex,
                )
                var program_dates = creEl(
                    "h4",
                    "program_dates",
                    "program_dates-" + currentIndex,
                )
                var supplementaryFooter = creEl(
                    "div",
                    "supplementary-footer",
                    "supplementary-footer-" + currentIndex,
                )
                tabContent.prepend(
                    program_dates,
            ...
            timer_clocks.setAttribute("data-style", "columns")
            timer_clocks.setAttribute("data-timezone", "America/Los_Angeles")
            timer_clocks.setAttribute("data-date", date)
            timer_clocks.setAttribute("data-digitscolor", "#a51c30")
    ...
    initiateTabs() {
        var d = document,
            ...
            stab = d.querySelectorAll(".stab_li"),
        ...
        stabs.addEventListener("click", function (e) {
            if (e.target && e.target.nodeName === "LI") {
                for (var i = 0; i < stab.length; i++) {
                    stab[i].classList.remove("active_tab")
                }
    ...
    /**
     * Get current active stab before initiate new supplementary
     */
    getCurrentActiveTag() {
    ...
    /**
	 If form is submitted/closed, set active stab to know which supplementary folder to display as open
	   */
    setCurrentActiveTag() {
        var $this = this
        ...
        const activeMainTabLi = document.getElementById(
            "li-" + $this.$activeMainTabID,
        )
        if (activeMainTabLi) {
