...
class AbandonedCartModal {
    ...
    checkAndDisplayModal() {
        ...
        if (cartData) {
            ...
            if (parsedCartData.createdOn && parsedCartData.programStartDate) {
                $this
                    .checkAndDisplayModals(parsedCartData)
                    .then((result) => {
                        $this.openModal()
                ...
                .then((data) => {
                    if (data.createdOn && data.programStartDate) {
                        ...
                        return Promise.reject(
                            "No createdOn or programStartDate found in the response.",
                        )
                    }
                })
                .then((result) => {
    ...
    checkAndDisplayModals(data) {
        return new Promise((resolve, reject) => {
            ...
            const sixHoursAgo = new Date(
                Date.now() - this.data.hour * 60 * 60 * 1000,
            )
            ...
            const isAbandonedModalOpen = localStorage.getItem(
                "isAbandonedModalOpen",
            )
            if (isAbandonedModalOpen === "true") {
                const lastOpenedDate = new Date(
                    localStorage.getItem("lastModalClosedDate"),
                )
                const now = new Date()
    ...
    displayCartMenuData() {
        ...
        const noRecordsDivs = document.querySelectorAll(
            "[data-cart-menu='no-records-div']",
        )
        const cartDataDivs = document.querySelectorAll(
            "[data-cart-menu='cart-data-div']",
        )
        const cartRedIcons = document.querySelectorAll(
            "[data-cart-menu='red-icon']",
        )
        ...
        if (
            !checkOutData.programStartDate ||
            !checkOutData.programEndDate ||
            !checkOutData.label ||
            !checkOutData.firstName ||
            !checkOutData.slug ||
            !checkOutData.programCategoryId
        ) {
            noRecordsDivs.forEach((div) => (div.style.display = "block"))
            cartDataDivs.forEach((div) => (div.style.display = "none"))
            return
        ...
        cartRedIcons.forEach((icon) => {
            icon.style.display = "block"
        })

        const programNameElements = document.querySelectorAll(
            "[data-cart-menu='programName']",
        )
        ...
        const programDateElements = document.querySelectorAll(
            "[data-cart-menu='programDate']",
        )
        ...
        const checkoutLinkElements = document.querySelectorAll(
            "[data-cart-menu='checkoutLink']",
        )
