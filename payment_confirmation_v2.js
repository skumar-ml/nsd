...
class PaymentConfirmation {
    ...
    handleLearnMoreClick(programDetailId) {
        ...
        let tutoring_data = this.$suppPro.filter(
            (i) =>
                i.label.toLowerCase().includes("tutoring") &&
                i.programDetailId === programDetailId,
        )

        let prep_week_data = this.$suppPro.filter(
            (i) =>
                i.label
                    .toLowerCase()
        ...
        let carePackageData = this.$suppPro.find(
            (i) => i.programDetailId === 21,
        )

        // Select modal elements
        let topicPrepUpSellModal = document.querySelector(
            ".upsell-modal-container.topic-prep-week",
        )
        ...
        if (programDetailId === 21 && carePackageData) {
            ...
            if (modal) {
                ...
                const closeLinks = document.querySelectorAll(
                    ".upsell-close-link.order-details",
                )
                if (closeLinks) {
    ...
    initializeProductSlider() {
        ...
        const productDetails = [
            {
                content:
                    "Limited Edition NSD Hoodie - Stay warm, look sharp, and rep NSD in this premium, unisex sweatshirt. Made from soft, high-quality fabric, it’s your new go-to for debate camp and beyond. Available only as part of the bundle, this hoodie is a must-have for every NSD student—grab yours before they’re gone!",
            },
            {
                content:
                    "The Timer Every Debater Needs - From practice rounds at camp to final rounds at tournaments, the NSD Timer is your go-to tool for staying in control of the clock and perfecting your speeches.",
            },
            {
                content:
                    "Hydration Made Simple - Stay hydrated in style with the NSD water bottle. Its durable metal build and striking red color make it the perfect companion for busy camp days and high-stakes tournaments.",
            },
            {
                content:
                    "NSD Hat: Where Comfort Meets Cool - Stay sharp and stylish with the unisex NSD hat, crafted for comfort and durability. Designed in a sleek, classic colorway, it’s the perfect accessory to rep your NSD pride—whether at camp, a tournament, or out and about.",
            },
            {
                content:
                    "NSD Tote Bag - This NSD tote’s sturdy, high-quality materials hold all your essentials, yet remain sleek and easy to carry. With its clean design and iconic NSD logo, it adds a polished touch to your routine—whether you’re headed to a debate round or a coffee shop.",
            },
        ]
        ...
        $sliderFor.on("afterChange", function (event, slick, currentSlide) {
            ...
            if (productInfoElement) {
                ...
                const productNameElement = productInfoElement.querySelector(
                    ".product-name_span-text",
                )
                console.log("Name", productNameElement)
                const productDescriptionElement =
                    productInfoElement.querySelector(".product-description-2")
    ...
    // Shows the specified upsell modal by adding show class
    ...
    // Hides the specified upsell modal by removing show class
    ...
    // Updates prep week modal with pricing and program information
    ...
    updateTutoringModal(tutoring_data) {
        if (tutoring_data.length > 0) {
            ...
            if (tutoringDescAmount != undefined) {
                tutoringDescAmount.innerHTML =
                    "$" + parseFloat(tutoring_data[0].disc_amount).toFixed(2)
            }
            var upSellTutoringProgramId = document.getElementById(
                "upsellTutoProgranId",
            )
            if (upSellTutoringProgramId != undefined) {
                upSellTutoringProgramId.setAttribute(
                    "programdetailid",
                    tutoring_data[0].programDetailId,
                )
                upSellTutoringProgramId.setAttribute(
                    "programName",
                    tutoring_data[0].label,
                )
                    ...
                    if (checkbox) {
                        ...
                        if (checkbox.checked) {
                            let programId =
                                ...
                                programName =
                                    checkbox.getAttribute("programName"),
    ...
    // Creates a checkout URL for supplementary program payment
    getPaymentUrl(programId, programName, amount) {
        ...
        const data = {
            ...
            successUrl:
                this.site_url + "portal/dashboard?paymentType=Supplementary",
            cancelUrl:
                this.site_url +
                "payment-confirmation?programId=" +
                encodeURIComponent(this.programId) +
                "&transactionID=" +
    ...
    async displaySupplementaryProgram() {
        ...
        let apiData = await this.fetchData(
            "/getSupplementaryProgram/" + this.programId,
        )
        ...
        allApiData.forEach((item) => {
            ...
            const updatedSlide = this.displaySingleSuppProgram(
                item,
                "desktop",
                swiperSlide,
            )
