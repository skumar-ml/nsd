    ...
    view() {
        ...
        if (this.$competition) {
            ...
            competition.forEach((singleCompetition) => {
                var accordionContainerDiv = creEl(
                    "div",
                    "accordion-container",
                    "accordion-container-" + $tabNo + $this.currentIndex,
                )
                var labelDiv = creEl("div", "label label-" + $this.currentIndex)
                // check forms for completion and put corresponding icon & text
                var textUncheck = $this.getCheckedText(
                    singleCompetition.is_live,
                )
...
class AccordionTabs {
    ...
    viewTabs(responseText) {
        ...
        responseText.forEach((formData, index) => {
            ...
            var activeLiClass =
                currentIndex == 1 && is_single ? "tab_li active_tab" : "tab_li"
            ...
            if (!is_single) {
                ...
                tabsE.innerHTML =
                    ...
                    " " +
                    formData.programDetail.debateEvent
            ...
            var accordionFooter = creEl(
                "div",
                "accordion-footer",
                "accordion-footer-" + currentIndex,
            )
    ...
    async getCompetitionsData() {
        ...
        const bgData = await $this.fetchData(
            PORTAL_API_BASE + "/getCompetitionDetails/" + $this.webflowMemberId,
        )
    ...
    displayNotification(notification) {
        ...
        notification.forEach((item) => {
            if (item.inTimeRange) {
                ...
                noText.innerHTML =
                    "Exciting news!  Self-check-in is live for  " +
                    item.programName +
                    ", and points are up for grabs!"
                notificationDiv.appendChild(noText)

                var live_com_budge =
                    document.getElementsByClassName("live_com_budge")[0]
