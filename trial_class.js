...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (wrapper && grid && template) {
    const updateCardSelectionClass = (container) => {
      ...
      optionLabels.forEach((optionLabel) => {
        const infoContainer = optionLabel.querySelector(".trial-class-info-container")
    ...
    try {
      const res = await fetch(`${window.NSD_API.TRIAL_CLASS_API_BASE}/trialClassDetail`)
      ...
      classes.forEach((item) => {
        ...
        if (radio) {
          ...
          radio.addEventListener("change", () => updateCardSelectionClass(grid))
        }
  ...
  if (gradeSelect) {
    try {
      const res = await fetch(`${window.NSD_API.TRIAL_CLASS_API_BASE}/getGrades`)
  ...
  form.addEventListener("submit", async function (e) {
    ...
    const selectedRadio = form.querySelector("input[name='trial-class']:checked")
    if (!selectedRadio) {
    ...
    const payload = {
      student: {
        first_name: form.querySelector("#student_first_name").value.trim(),
      ...
      previous_experience: form.querySelector("#previous_experience").value.trim(),
    ...
    try {
      ...
      setTimeout(() => {
        if (failBox) failBox.style.setProperty("display", "none", "important")
        if (successBox) successBox.style.setProperty("display", "block", "important")
      }, 50)
  ...
  if (manageToken) {
    ...
    if (rescheduleBtn && registrationCard && rescheduleForm) {
      rescheduleBtn.addEventListener("click", function () {
        ...
        const firstVisibleRescheduleRadio = rescheduleForm.querySelector("input[name='res-trial-class']")
      ...
      cancelBtn.addEventListener("click", function () {
        ...
        fetch(cancelUrl, {
          ...
          .then((res) => {
            ...
            return res.text().then((t) => (t && t.trim() ? JSON.parse(t) : {}))
          })
          .then(() => {
            const infoCard = document.querySelector(".tc_cancel-registration-info-card")
            const doneCard = document.querySelector(".tc_registration-cancelled-card")
            if (infoCard) {
    ...
    if (rescheduleForm) {
      ...
      if (studentGrade && studentGrade.tagName === "SELECT") {
        if (gradeSelect && gradeSelect.options.length > 1) {
          ...
          try {
            const gradeRes = await fetch(`${window.NSD_API.TRIAL_CLASS_API_BASE}/getGrades`)
      ...
      if (rescheduleSuccessBox) rescheduleSuccessBox.style.setProperty("display", "none", "important")
      if (rescheduleFailBox) rescheduleFailBox.style.setProperty("display", "none", "important")
      ...
      rescheduleForm.addEventListener("submit", async function (e) {
        ...
        const selectedRadio = rescheduleForm.querySelector("input[name='res-trial-class']:checked")
        if (!selectedRadio) {
          alert("Please select a trial class.")
          return
        }
        ...
        try {
          ...
          setTimeout(() => {
            if (rescheduleFailBox) rescheduleFailBox.style.setProperty("display", "none", "important")
            if (rescheduleSuccessBox) rescheduleSuccessBox.style.setProperty("display", "block", "important")
          }, 50)
