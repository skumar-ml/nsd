...
class NSDPortal {
        ...
        if (
          ...
          if (activeStudentTab) {
            const studentTabIndex = Array.from(
              tabsContainer.querySelectorAll(".student-program-info-link"),
  ...
  createStudentTabHeader(studentData, tabIndex, isActive, studentIndex) {
    ...
    const currentSessions = studentData.sessions.filter(
      (s) =>
        s.sessionType === "current" && s.programDetail?.programDetailId != 21,
    )
    const programNames = [
      ...new Set(
        currentSessions
  ...
  initializeStripePayment(
    ...
    const data = {
      ...
      successUrl:
        "https://www.nsdebatecamp.com/portal/dashboard?programName=" + title,
