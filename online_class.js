...
function renderPane(pane, result) {
  ...
  terms.forEach((term) => {
    ...
    if (dateEl) dateEl.textContent = `${formatDate(term.start_date)} – ${formatDate(term.end_date)}`
    ...
    if (countEl) {
      countEl.textContent = classes.length === 1 ? "1 Class" : `${classes.length} Classes`
    }

    const cardContainer = termClone.querySelector(".oc-course-card-container")
    const cardTemplate = cardContainer?.querySelector(".online-class_course-card")
    ...
    classes.forEach((course) => {
      ...
      if (registerBtn) {
        ...
        url.searchParams.set("grade_id", result.data?.[0]?.grade_id || "")
        url.searchParams.set("grade_name", result.data?.[0]?.gradeLabel || "")
        url.searchParams.set("session_name", sessions[0]?.session_name || "")
        url.searchParams.set("session_detail_id", term.session_detail_id || "")
        url.searchParams.set("class_detail_id", course.class_detail_id || "")
        registerBtn.href = url.toString()
      }

      const titleEl = card.querySelector(".online-class_course-tittle")
      const descEl = card.querySelector(".online-class_course-description p:last-child")
      const feeEl = card.querySelector(".online-class_fee")

      if (titleEl) titleEl.textContent = course.className || "Untitled Course"
      if (descEl) descEl.textContent = course.description || ""
      if (feeEl) feeEl.textContent = course.price != null ? `$${course.price}` : "—"
      ...
      const sessionsGrid = card.querySelector(".online-class_sessions-grid")
      const slotTemplate = sessionsGrid?.querySelector(".online-class_slot-card")
      ...
      if (sessionsGrid && slotTemplate) {
        ...
        schedules.forEach((schedule) => {
          ...
          const timeEl = slot.querySelector(".online-class_session-time")
          const spotsEl = slot.querySelector(".online-class_red-text")
          const spotWrapper = slot.querySelector(".online-class_spot-wapper")

          const shortDay = schedule.day?.slice(0, 3) || ""
          const availableSpots = schedule.available_spots ?? 0

          if (timeEl) timeEl.textContent = `${shortDay} ${utcToEST(schedule.start_time) || ""}`
          if (spotsEl) spotsEl.textContent = availableSpots

          if (spotWrapper) {
            spotWrapper.style.display = availableSpots <= 3 ? "flex" : "none"
          }
