...
class Notification {
    ...
    displayUnreadMessage() {
        var notificationBudge =
            document.getElementsByClassName("notification-budge")[0]
        var notificationCount =
            document.getElementsByClassName("notification-count")[0]
    ...
    getMessageType() {
        return this.filterData.filter(
            (obj, index) =>
                this.filterData.findIndex((item) => item.type == obj.type) ===
                index,
        )
    }
    ...
    filterMessageData() {
        ...
        if (searchFilter.value) {
            ...
            messageData = messageData.filter(function (el) {
                return (
                    condition.test(el.title) ||
                    condition.test(el.message) ||
                    condition.test(el.type) ||
                    condition.test(el.sendAs)
                )
            })
        }
    ...
    createMessageTypeFilter() {
        ...
        var messagetype = creEl(
            "select",
            "message-type-filter w-select",
            "message-type-filter",
        )
    ...
    createSearchFilter() {
        var $this = this
        var col = creEl("div", "col")
        var searchFilter = creEl(
            "input",
            "search-filter form-text-label w-input",
            "search-filter",
        )
    ...
    downLoadLinkIcon(fileLink, type = "") {
        ...
        if (type == "download") {
            ...
            var img = creEl("img")
            img.src =
                "https://uploads-ssl.webflow.com/6271a4bf060d543533060f47/642e8f4e85af6b0ef1e409b6_link-solid.png"
        }
        a.appendChild(img)
    ...
    viewDownLoadedFile(fileLink) {
        ...
        img.src =
            "https://uploads-ssl.webflow.com/6271a4bf060d543533060f47/643501a495c54e74d70e60ba_view-file.svg"
        img.title = "View"
        a.appendChild(img)
        a.href = fileLink
    ...
    showListPage() {
        ...
        var notification = document.getElementById("notification")
        var notificationDetails = document.getElementById(
            "notification-details",
        )
        var notificationHeading = document.getElementsByClassName(
    ...
    detailPageBackButton(className) {
        ...
        img.src =
            "https://uploads-ssl.webflow.com/6271a4bf060d543533060f47/6434fd8a3e27f65f1168c15b_arrow-left.svg"
        img.title = "Back"
            ...
            if (item.uploadedFiles) {
                if (Array.isArray(item.uploadedFiles)) {
                    item.uploadedFiles.forEach((url) => {
                        var downloadIcon = this.downLoadLinkIcon(
                            url,
                            "download",
                        )
    ...
    readApiCall(messageId) {
        ...
        xhr.open(
            "POST",
            "https://3yf0irxn2c.execute-api.us-west-1.amazonaws.com/dev/camp/isReadNotification",
            true,
        )
    ...
    createPagination() {
        ...
        if (this.paginateData.pre_page != null) {
            ...
            preBtn.addEventListener("click", function () {
                $this.paginateData = $this.paginatorList(
                    $this.messageData,
                    $this.paginateData.pre_page,
                )
                $this.refreshData()
            })
    ...
    makeMessageFilter() {
        var notificationFilter = document.getElementById("notification-filter")
        /*Filter*/
        var notificationHeader = creEl(
            "div",
            "notification-header w-layout-grid grid-3",
            "notification-header",
        )
...
class NotificationApi {
    ...
    getNotificationData() {
        ...
        xhr.open(
            "GET",
            "https://3yf0irxn2c.execute-api.us-west-1.amazonaws.com/dev/camp/getNotifications/" +
