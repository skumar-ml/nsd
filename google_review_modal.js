...
class GoogleReviewModal {
    ...
    constructor(memberData) {
        ...
        this.closeButtons = document.querySelectorAll(
            ".google-review-close-link, #may-be-later",
        )
    ...
    getTestOverrides() {
        ...
        return {
            ...
            forceShow:
                params.get("forceShow") === "1" ||
                params.get("forceShow") === "true",
        }
    }
    ...
    clearGoogleReviewData() {
        if (!this.googleReviewData || !this.googleReviewData.studentEmail)
            return
    ...
    isEligibleForReview() {
        ...
        if (
            this.googleReviewData &&
            Object.keys(this.googleReviewData).length > 0
        ) {
            this.checkConditionsAndShowModal()
        } else {
            // Fetch program data
            fetch(`${FORMS_API_BASE}/getCompletedForm/${this.memberId}/current`)
                ...
                .then((campData) => {
                    ...
                    const currentStudentData = campData.find(
                        (data) =>
                            data.studentDetail.studentEmail ===
                            this.studentEmail,
                    )
                    ...
                    const { startDate, endDate } =
                        currentStudentData.programDetail
    ...
    updateGoogleReviewData(data) {
        ...
        localStorage.setItem(
            "googleReviewData",
            btoa(JSON.stringify(this.googleReviewData)),
        )
    }
    ...
    checkConditionsAndShowModal() {
        ...
        const isHalfwayThrough =
            completionPercentage >= 50 && currentDate < endDate
