...
class DisplaySuppProgram {
    ...
    async displaySupplementaryProgram() {
        ...
        let apiData = await this.fetchData(
            this.paymentApiBase,
            "/getSupplementaryProgram/" + this.memberData.memberId,
        )
        ...
        apiData.forEach((item) => {
            ...
            if (item.disc_amount && item.portal_amount) {
                ...
                // Modal Content
                var outerShadowDivM = this.displaySingleSuppProgramB(
                    item,
                    "modal",
                )
        ...
        var slideCheckInterval = setInterval(
            () =>
                this.visibilityTimeout(swiperSliderParent, slideCheckInterval),
            500,
        )
    }
    ...
    visibilityTimeout(swiperSliderParent, slideCheckInterval) {
        let slideItems = document.querySelector(
            ".discounted-programs-slide-item",
        )
        if (slideItems) {
    ...
    displaySingleSuppProgram(item) {
        ...
        var slideItem = $this.creEl(
            "div",
            "discounted-programs-slide-item " + bannerTypeClass,
        )
        // Image wrapper
        ...
        var programsDiv = $this.creEl(
            "div",
            "discounted-programs-div summer-program",
        )
        ...
        discountPrice.innerHTML = item.portal_amount
            ? "$" + item.portal_amount
            : "" + "<br />"
        ...
        if (benefits.length > 0) {
            var learnMoreBtn = this.creEl(
                "a",
                "main-button learn-more w-button",
            )
            learnMoreBtn.href = "#"
    ...
    displaySingleSuppProgramB(item, type = "banner") {
        ...
        let bannerTypeClass =
            "banner-content " + type + "-" + item.programDetailId
        // Main wrapper
        if (type == "banner") {
            var slideItem = $this.creEl(
                "div",
                "supp-programs-slide-item " + bannerTypeClass,
            )
        } else {
            var slideItem = $this.creEl(
                "div",
        ...
        var programsDiv = $this.creEl(
            "div",
            "discounted-programs-div border-none",
        )
        ...
        // Price Grid
        var priceGrid = $this.creEl(
            "div",
            "discount-price-grid supp-prog-price",
        )

        var originalPrice = $this.creEl(
            "div",
            "original-price-gray medium-text",
        )
        originalPrice.textContent = "$" + item.disc_amount
        ...
        if (benefits.length > 0) {
            benefits.forEach(function (benefit) {
                ...
                var benefitTitle = $this.creEl("div", "dm-sans benefits-text")
        ...
        var limitedTime = $this.creEl(
            "div",
            "dm-sans limited-time-supp-program",
        )
    ...
    initSlickSlider() {
        var $slider = $(".discounted-programs-slick-slider")
        ...
        var $slider2 = $(".supp-programs-slick-slider")
        // Check if the slider is already initialized
    ...
    initSupplementaryPayment(paymentId, programId, programName, amount) {
        ...
        const data = {
            ...
            successUrl:
                ...
                "portal/dashboard?paymentType=portal",
        ...
        if (upSellAmount.length > 0) {
            upSellAmount.forEach((up_Sell_price) => {
                up_Sell_price.innerHTML =
                    "$" + $this.$selectedProgram.portal_amount
            })
        }
